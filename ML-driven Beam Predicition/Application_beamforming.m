classdef Application_beamforming < matlab.apps.AppBase
    ...
    TitleDNN
        ...
        UITableDNN      matlab.ui.control.Table
        UITableTree      matlab.ui.control.Table
        UITableKNN      matlab.ui.control.Table
        UIAxesBS       matlab.ui.control.UIAxes
        UIAxesUT       matlab.ui.control.UIAxes
    ...
    k_x        matlab.ui.control.NumericEditField
      k_y       matlab.ui.control.NumericEditField
    end    

    methods (Access = public)
        function app = Application_beamforming()
            % Create UI Figure & Components
            app.UIFigure = uifigure('Name', 'Beamforming Optimization', 'Position', [100, 100, 900, 600]);
            ...
            app.LOS_ConditionField = uidropdown(app.UIFigure, 'Position', [140, 550, 50, 30], 'Items', {'LoS', 'NLoS'}, 'Value', 'LoS');

            % Label and Input Fields (adjusted for spacing)
            % Base Station height
            app.h_BSLabel = uilabel(app.UIFigure, 'Text', 'h_BS (m)', 'Position', [20, 500, 100, 30]);
            app.h_BSField = uieditfield(app.UIFigure, 'numeric', 'Position', [120, 500, 50, 30]);

            % User Terminal height
            app.h_UTLabel = uilabel(app.UIFigure, 'Text', 'h_UT (m)', 'Position', [20, 460, 100, 30]);
            app.h_UTField = uieditfield(app.UIFigure, 'numeric', 'Position', [120, 460, 50, 30]);

            % Base Station x and y coordinates
            app.xBS_0Label = uilabel(app.UIFigure, 'Text', 'xBS_0', 'Position', [20, 420, 100, 30]);
            app.xBS_0Field = uieditfield(app.UIFigure, 'numeric', 'Position', [120, 420, 50, 30]);
            app.yBS_0Label = uilabel(app.UIFigure, 'Text', 'yBS_0', 'Position', [20, 380, 100, 30]);
            app.yBS_0Field = uieditfield(app.UIFigure, 'numeric', 'Position', [120, 380, 50, 30]);

            % User Terminal x and y coordinates
            app.xUT_0Label = uilabel(app.UIFigure, 'Text', 'xUT_0', 'Position', [20, 340, 100, 30]);
            app.xUT_0Field = uieditfield(app.UIFigure, 'numeric', 'Position', [120, 340, 50, 30]);
            app.yUT_0Label = uilabel(app.UIFigure, 'Text', 'yUT_0', 'Position', [20, 300, 100, 30]);
            app.yUT_0Field = uieditfield(app.UIFigure, 'numeric', 'Position', [120, 300, 50, 30]);

            % Antenna Fields
            app.Nrx_hLabel = uilabel(app.UIFigure, 'Text', 'Nrx_h', 'Position', [300, 500, 100, 30]);
            app.Nrx_hField = uieditfield(app.UIFigure, 'numeric', 'Position', [400, 500, 50, 30]);
            app.Nrx_vLabel = uilabel(app.UIFigure, 'Text', 'Nrx_v', 'Position', [300, 460, 100, 30]);
            app.Nrx_vField = uieditfield(app.UIFigure, 'numeric', 'Position', [400, 460, 50, 30]);
            app.Ntx_hLabel = uilabel(app.UIFigure, 'Text', 'Ntx_h', 'Position', [300, 420, 100, 30]);
            app.Ntx_hField = uieditfield(app.UIFigure, 'numeric', 'Position', [400, 420, 50, 30]);
            app.Ntx_vLabel = uilabel(app.UIFigure, 'Text', 'Ntx_v', 'Position', [300, 380, 100, 30]);
            app.Ntx_vField = uieditfield(app.UIFigure, 'numeric', 'Position', [400, 380, 50, 30]);

            % Time-related Fields
            app.TLabel = uilabel(app.UIFigure, 'Text', 'Number Slots', 'Position', [300, 340, 100, 30]);
            app.TField = uieditfield(app.UIFigure, 'numeric', 'Position', [400, 340, 50, 30]);
            app.UpdatePeriodLabel = uilabel(app.UIFigure, 'Text', 'Update Period (s)', 'Position', [300, 300, 100, 30]);
            app.UpdatePeriodField = uieditfield(app.UIFigure, 'numeric', 'Position', [400, 300, 50, 30]);

            % Speed and Frequency Fields
            app.SpeedUELabel = uilabel(app.UIFigure, 'Text', 'Speed UE (km/h)', 'Position', [580, 500, 100, 30]);
            app.SpeedUEField = uieditfield(app.UIFigure, 'numeric', 'Position', [680, 500, 50, 30]);
            app.fcLabel = uilabel(app.UIFigure, 'Text', 'Carrier Frequency (Hz)', 'Position', [580, 460, 150, 30]);
            app.fcField = uieditfield(app.UIFigure, 'numeric', 'Position', [680, 460, 50, 30]);
            app.snrLabel = uilabel(app.UIFigure, 'Text', 'SNR transmission (Linear)', 'Position', [580, 420, 150, 30]);
            app.snrField = uieditfield(app.UIFigure, 'numeric', 'Position', [680, 420, 50, 30]);
             app.kLabel = uilabel(app.UIFigure, 'Text', 'Codebook Size', 'Position', [580, 380, 150, 30]);
            app.k_x = uieditfield(app.UIFigure, 'numeric', 'Position', [680, 380, 20, 30]);
            app.k_y = uieditfield(app.UIFigure, 'numeric', 'Position', [720, 380, 20, 30]);
          % Run Button
app.RunButton = uibutton(app.UIFigure, 'push', 'Text', 'Run', 'Position', [700, 320, 100, 50]);
app.RunButton.ButtonPushedFcn = @(btn, event) runSimulation(app);


            % Création de la première table (Métriques)
app.UITableMetrics = uitable(app.UIFigure, 'Position', [20, 50, 400, 20]);

% Création de la deuxième table (Matrice H)
app.UIAxesBS = uiaxes(app.UIFigure, 'Position', [20, 100, 400, 50]); 
app.UIAxesUT = uiaxes(app.UIFigure, 'Position', [20, 180, 400, 50]); 
            % Axes for Graph
            app.UITableTree = uitable(app.UIFigure, 'Position', [450, 75, 400, 30]);
            app.TitleTree = uilabel(app.UIFigure, 'Text', 'Evaluation Metrics Decision Tree', ...
    'Position', [450, 105, 400, 30])%, 'FontSize', 14, 'FontWeight', 'bold');
            app.UITableKNN = uitable(app.UIFigure, 'Position', [450, 135, 400, 30]);
            app.TitleKNN = uilabel(app.UIFigure, 'Text', 'Evaluation Metrics KNN', ...
    'Position', [450, 165, 400, 30])%, 'FontSize', 14, 'FontWeight', 'bold');
            app.UITableDNN = uitable(app.UIFigure, 'Position', [450, 195, 400, 30]);
            app.TitleDNN = uilabel(app.UIFigure, 'Text', 'Evaluation Metrics DNN', ...
    'Position', [450, 225, 400, 30])%, 'FontSize', 14, 'FontWeight', 'bold');
...
app.UIAxesBS.XTick = [];  % Remove X-axis ticks
app.UIAxesBS.YTick = [];  % Remove Y-axis ticks
app.UIAxesUT.XTick = [];  % Remove X-axis ticks
app.UIAxesUT.YTick = [];  % Remove Y-axis ticks
            % Titre pour la zone des métriques
app.TitleMetricsLabel = uilabel(app.UIFigure, 'Text', 'Evaluation Metrics', ...
    'Position', [20, 70, 400, 30])%, 'FontSize', 14, 'FontWeight', 'bold');

% Titre pour la zone de la matrice du canal
app.TitleChannelLabel = uilabel(app.UIFigure, 'Text', 'Power for each beam at BS', ...
    'Position', [20, 150, 400, 30])%, 'FontSize', 14, 'FontWeight', 'bold');

% Titre pour la zone de la matrice du canal
app.Titlezone1 = uilabel(app.UIFigure, 'Text', 'Codebook-based Selection', ...
    'Position', [100, 260, 400, 30], 'FontSize', 14, 'FontColor', [0,0,1], 'FontWeight', 'bold');
app.Titlezone2 = uilabel(app.UIFigure, 'Text', 'ML-Beam Prediction', ...
    'Position', [600, 260, 400, 30], 'FontSize', 14, 'FontColor', [0,0,1], 'FontWeight', 'bold');
% Titre pour la zone des figures
app.TitleFigureLabel = uilabel(app.UIFigure, 'Text', 'Power for each beam at UE', ...
    'Position', [20, 230, 400, 30])%, 'FontSize', 14, 'FontWeight', 'bold');
...
for t=1:T
 ...
 % Update location (example: moving UE)
    ...
    [beamBS_opt,beamAzBS_opt,beamElBS_opt,beamAngleBS,beamAngleAzBS,beamAngleElBS] = getDFTCodebook3D(Ntx_h,Ntx_v,kx,ky,0.5,0.5);
 [beamUE_opt,beamAzUE_opt,beamElUE_opt,beamAngleUE,beamAngleAzUE,beamAngleElUE] = getDFTCodebook3D(Nrx_h,Nrx_v,kx,ky,0.5,0.5);
...
tb = beam_Ex(1,1);
rb = beam_Ex(1,2);
beam_indices = 1:kx*ky; % Indices des faisceaux
x = SE_All(:,rb)
bar(app.UIAxesBS, beam_indices, SE_All(:,rb));
    xlabel(app.UIAxesBS, 'Beam Index');
    ylabel(app.UIAxesBS, 'Power (Watt)');
   % text(beam_indices, SE_All(:,rb), string(SE_All(:,rb)), 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom');
 bar(app.UIAxesUT, beam_indices, SE_All(tb,:));
    xlabel(app.UIAxesUT, 'Beam Index');
    ylabel(app.UIAxesUT, 'Power (Watt)'); 
   % text(beam_indices, SE_All(tb,:), string(SE_All(tb,:)), 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom');
 i = i+1;
SNR_linear(1,t) = 1;
dr_ExOpt = computeDataRate_t(t,1,beam_Ex,freqChan,beamBS_opt,beamUE_opt,1,SNR_linear,1);
% Update the table with current values
    metrics = table(tb, rb, dr_ExOpt, elapsed_time, ...
                    'VariableNames', {'Best Beam at BS', 'Best Beam at UE', 'Spectral Efficiency (bps/Hz)', 'Execution Time (s)'});
...
Channel_2(itr,:,:,t,k) = Channel_2(itr,:,:,t,k)./norm(chan);
    X = reshape(Channel_2(itr,:,:,t,1), 1, []);
    X_in(1,1:16) = real(X);
X_in(1,17:32) = imag(X);
X_in(1,33:48) = abs(X);
X_in(1,49:64) = angle(X);
load('dnnModel.mat', 'dnn');
tic  % Start the timer
tb_dnn = double(classify(dnn, X_in))
time_dnn = toc;  % Stop the timer and store the elapsed time
load('dnnModel_rx.mat', 'dnn');
%tic
rb_dnn = double(classify(dnn, X_in))
%time_dnn = time_dnn+ toc;
f = beamBS_opt(:,tb_dnn);
   % f = repelem(f,numPolAng,1);
    w = beamUE_opt(:,rb_dnn);
  %  w = repelem(w,numPolAng,1);
    H = freqChan(:,:,1,t);
se_dnn = abs(log2(1+snr*abs(10*log10(w'*H*f*f'*H'*w))));
metrics = table(tb_dnn, rb_dnn, se_dnn, time_dnn*10^(-3), ...
                    'VariableNames', {'Best Beam at BS', 'Best Beam at UE', 'Spectral Efficiency (bps/Hz)', 'Execution Time (s)'});
      app.UITableDNN.Data = metrics; 


      load('knn_model_tx.mat', 'knn_model');
tic  % Start the timer
tb_knn = predict(knn_model, X_in)
time_knn = toc;  % Stop the timer and store the elapsed time
load('knn_model_rx.mat', 'knn_model');
%tic
rb_knn = predict(knn_model, X_in)
%time_knn = time_knn+ toc;
f = beamBS_opt(:,tb_knn);
   % f = repelem(f,numPolAng,1);
    w = beamUE_opt(:,rb_knn);
  %  w = repelem(w,numPolAng,1);
    H = freqChan(:,:,1,t);
se_knn = abs(log2(1+snr*abs(10*log10(w'*H*f*f'*H'*w))));
metrics = table(tb_knn, rb_knn, se_knn, time_knn*10^(-3), ...
                    'VariableNames', {'Best Beam at BS', 'Best Beam at UE', 'Spectral Efficiency (bps/Hz)', 'Execution Time (s)'});
      app.UITableKNN.Data = metrics; 


         load('tree_model.mat', 'tree_model');
tic  % Start the timer
tb_tree = predict(tree_model, X_in)
time_knn = toc;  % Stop the timer and store the elapsed time
load('tree_model_Rx.mat', 'tree_model_Rx');
%tic
rb_tree = predict(tree_model_Rx, X_in)
%time_knn = time_knn+ toc;
f = beamBS_opt(:,tb_tree);
   % f = repelem(f,numPolAng,1);
    w = beamUE_opt(:,rb_tree);
  %  w = repelem(w,numPolAng,1);
    H = freqChan(:,:,1,t);
se_tree = abs(log2(1+snr*abs(10*log10(w'*H*f*f'*H'*w))));
metrics = table(tb_tree, rb_tree, se_tree, time_knn*10^(-3), ...
                    'VariableNames', {'Best Beam at BS', 'Best Beam at UE', 'Spectral Efficiency (bps/Hz)', 'Execution Time (s)'});
      app.UITableTree.Data = metrics; 
